...
4. Adapte as dicas considerando os dados do clima, o horário da pedalada, o nível do ciclista e o estilo escolhido.
    ...
    try:
        ...
        with st.spinner("Gerando rota personalizada..."):
            ...
            if geocode_result:
                    ...
                    for waypoints in waypoint_sets:
                        try:
                            ...
                            if test_route:
                                ...
                                # Mais rigoroso: rota deve estar dentro de 1.8km da distância solicitada
                                if distance_diff <= 1.8 and test_distance <= distancia + 2.0:
                                    # Encontramos uma rota dentro do limite, usar imediatamente
                ...
                # Se não conseguiu com pontos específicos, usar a melhor rota circular calculada
...
def main():
    ...
    # Página de Resultados
        ...
        # Criar cards simples para os sensores
        sensor_cols = st.columns(4, gap="small")
        mapping = {
        ...
        for (label,(k,u)), col in zip(mapping.items(), sensor_cols):
            with col:
                if k in dados:
                    v = dados[k]
                    st.markdown(
        ...
        with st.spinner("2/6: Preparando rota personalizada..."):
            # Rotas seguras adaptadas ao perfil do ciclista em São José dos Campos
            ...
            # Gerar a rota completa primeiro
            mapa_html, texto_completo, elevation_data = gerar_rota_e_embed(data['endereco'], pontos_rota, data['distancia'])
